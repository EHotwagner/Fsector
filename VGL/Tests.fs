...
module Tests =
    ...
    let triangle1 = sh.triangleIsoc 100 50 st.BlackF (tr.position 200 200)
    let byTriangle = sh.triangleIsoc 100 50 st.BlackF (tr.position 200 200) |> im.Figure
    let rgTriangle = sh.triangleIsoc 100 50 st.BlackF (tr.position 200 200) |> im.Figure
    ...
    let path3 = im.path (st.outlined Color.Red Stroke.Medium) (Path.path (V(299.,182.)) [Utilities.CBezier(V(299.,182.),V(294.,176.),V(285.,178.)); Utilities.CBezier(V(276.,179.),V(272.,173.),V(272.,173.))] Utilities.FigureBegin.Hollow Utilities.FigureEnd.Open FillMode.Alternate) (tr.scale 10. (tr.position 200 200)) 
